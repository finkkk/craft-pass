...
- 后台控制申请入口、答题失败冷却时间、同 IP/QQ/Minecraft ID 提交频率限制
- 自定义 RCON 命令开关和危险命令黑名单
- 恢复出厂设置，可清空数据并回到首次部署流程
- 玩家端题目按访问随机排序，后端仍按题目标识独立判分
- 申请量、通过率、状态分布和最近趋势统计
...
`npm start` 会自动构建最新的前端和后端，然后由后端在同一个 `3000` 端口提供前端网页与 API。
...
- 题库与服规：`http://localhost:3000/admin/content`
- 界面定制：`http://localhost:3000/admin/appearance`
- RCON 控制台：`http://localhost:3000/admin/rcon`
- 系统配置：`http://localhost:3000/admin/settings`

RCON 可以在首次部署时暂时关闭，之后再从系统配置页面启用。未启用或连接失败时仍可查看、管理和拒绝申请，但不能执行白名单批准操作或自定义 RCON 命令。系统配置页还可以临时关闭玩家申请入口、设置失败冷却和提交频率限制。“提交统计时段”只用于统计短时间内同 IP/QQ/Minecraft ID 的提交次数，不是玩家答题限时。
...
日常部署和使用只需要访问 `http://localhost:3000`。前端开发服务器仅用于源码热更新调试，不作为普通访问入口。
