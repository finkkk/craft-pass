...
上述开发命令只用于源码热更新调试。日常部署和验收请直接访问 `http://localhost:3000`，该端口会同时提供网页和 API。
...
完成初始化后，可以在 `/admin/appearance` 修改站点名称、Logo 和玩家端文案，在 `/admin/content` 修改服规与题库，在 `/admin/settings` 修改申请入口、答题失败冷却、同 IP/QQ/Minecraft ID 提交频率限制、RCON 主机、端口、密码、超时、白名单命令、自定义命令开关和危险命令黑名单。这里的“提交统计时段”不是答题限时，只用于统计短时间内的重复提交次数。RCON 控制台位于 `/admin/rcon`，连接成功且允许自定义命令后可以发送命令并查看服务端输出。新设置会立即用于后续操作，无需重启后端。
