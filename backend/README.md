...
玩家通过 QQ 与 Minecraft ID 的精确组合查询审核状态。注册时会分别检查 QQ 和不区分大小写的 Minecraft ID；任何状态的历史申请都会占用身份，包括答题失败和已拒绝记录。管理员拒绝申请时原因可留空，填写后会展示给对应玩家。管理员删除占用记录后，该 QQ 与 Minecraft ID 会立即释放。

新建和经管理员编辑过的申请还会写入 `identity_locked`，由数据库部分唯一索引防止并发请求造成重复绑定。升级前已有的记录保持未锁定，避免历史重复数据导致迁移失败，但应用层仍会把它们纳入查重；管理员可以在后台逐条修改或删除冲突记录。
