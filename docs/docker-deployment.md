...
- `CORS_ORIGINS` 必须等于浏览器实际访问来源，例如 `https://apply.example.com`。
- `APP_SECRET` 必须替换为至少 32 位随机值，可用 `openssl rand -base64 48` 生成。
- `APP_PORT` 是应用绑定到宿主机环回地址的端口，默认 `47821`。
- RCON 未启用时保持 `RCON_ENABLED=false`；Minecraft 位于同一宿主机时使用 `RCON_HOST=host.docker.internal`。
- 只有启用内置 Caddy 时才需要配置 `SITE_ADDRESS`。
...
确保宿主机 80/443 没有被 Nginx、Apache 等服务占用。在 `.env` 中设置：
